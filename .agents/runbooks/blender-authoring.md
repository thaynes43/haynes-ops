...
The earlier dev-env image build in [PR #2831](https://github.com/thaynes43/haynes-ops/pull/2831) validated the local toolchain but was superseded by Tom's request for independent workloads. Do not activate that image to obtain Blender. Both Blender/audio MCP registrations and provider-specific agent startup rules are staged through merged [PR #2844](https://github.com/thaynes43/haynes-ops/pull/2844), with the two startup ConfigMaps temporarily excluded from Reloader by [#2843](https://github.com/thaynes43/haynes-ops/pull/2843). Their live contents match Git without a dev-env restart. Held [PR #2833](https://github.com/thaynes43/haynes-ops/pull/2833) restores normal reload behavior and adds one explicit pod-template marker, so Tom merges it at a natural break for the single planned restart. Later Blender image/config upgrades do not touch dev-env resources.
...
One upgrade trap to know before bumping `BLENDER_MCP_REVISION`: `service.py` and `test-service.py` here still use the **mcp 1.x** Python API (`mcp.server.fastmcp.FastMCP`, `streamablehttp_client`), which mcp 2.x removes outright. Renovate cannot reach that version — the SDK comes from upstream blender-mcp's own `uv.lock` via `uv sync --frozen` at a pinned git revision — so the pin is safe today. But a revision bump whose lock has moved to mcp 2.x would fail at import with `ModuleNotFoundError: No module named 'mcp.server.fastmcp'`. The equivalent migration for the audio service is [PR #3001](https://github.com/thaynes43/haynes-ops/pull/3001) (`FastMCP` → `MCPServer`, transport knobs moved onto `streamable_http_app()`, `streamablehttp_client` → `streamable_http_client` yielding a 2-tuple, `CallToolResult.isError` → `.is_error`).
