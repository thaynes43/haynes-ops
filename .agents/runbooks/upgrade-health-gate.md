...
| **Benign-warn** | HEALTH_WARN allowlist — do **not** page: `N daemons have recently crashed` from stale/archived night crashes (the recurring ~02:30 ET mon.i tcmalloc / osd.2 perf-counter aborts, archived per id by the `rem-*` lane after it verifies recovery — see [`ceph-daemon-crash.md`](ceph-daemon-crash.md); `ceph crash archive-all` remains the human break-glass); mon clock skew; maintenance flags (`noout`/`norebalance`) set during a roll; transient degraded% / `recovering` / `backfilling` as OSDs roll one-node-at-a-time during a rook upgrade (degraded spikes then recovers — `watchrook.sh` pattern). |
