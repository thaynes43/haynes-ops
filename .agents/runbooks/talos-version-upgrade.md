...
If a node sits in draining for >10min, it's almost always a PDB with
`ALLOWED DISRUPTIONS: 0` refusing evictions — see
[Drain blockers (PDBs)](#drain-blockers-pdbs-node-stuck-draining) below.
...
### Drain blockers (PDBs): node stuck draining

Every stall in the 2026-08-21 v1.13.3→v1.13.9 roll was a `minAvailable: 1` (or
equivalent) PDB on a **single-replica** pod — ALLOWED DISRUPTIONS 0, so the
eviction API refuses forever and Omni's drain waits indefinitely.

**Find the stragglers** (what's still on the node, minus DaemonSets — those
don't drain):
...
kubectl get pods -A --field-selector spec.nodeName=<node> \
  ...
  | grep -v DaemonSet
# Which PDBs are eviction-proof right now:
kubectl get pdb -A | awk '$5 == 0'   # ALLOWED DISRUPTIONS column = 0
```

**Unstick, by pod class:**

- **Stateless / movable** (Deployment-backed, Ceph or no storage) →
  `kubectl delete pod -n <ns> <pod>`. Delete bypasses the eviction API (PDBs
  only guard evictions); the replacement schedules on another node.
- **CNPG singleton on openebs-hostpath** (e.g. `postgres16-pgvecto`,
  `ytdrivarr-postgres16`) → check the last backup first
  (`kubectl get scheduledbackup,backup -n <ns>`), then `kubectl delete pod`.
  The replacement sits **Pending until the node returns** (hostpath PVC pins
  it) — that's expected and fine for an in-place upgrade: `/var` survives, the
  pod resumes with its data when the node is back.
- **CNPG multi-instance with the PRIMARY on the draining node** (e.g.
  `postgres16`) → **switchover first, never delete the primary**:
  `kubectl cnpg promote <cluster> <replica-instance> -n <ns>` (plugin at
  `~/.local/bin/kubectl-cnpg`), wait for the promotion to settle, then the old
...
- **`emqx-core`** (database ns) → its PDB is generated by the emqx-operator
  with neither minAvailable nor maxUnavailable → permanently ALLOWED 0, and
  the 2.x EMQX CR exposes **no PDB knob**. `kubectl delete pod` it; retained
  MQTT state re-publishes on restart (Zigbee2MQTT may need a restart after —
  see CLAUDE.md cascading-restarts note). Operator 3.0 (held, see
  `.renovate/holds.json5`) should be checked for PDB config when the hold lifts.

**Durable fixes in place since 2026-08-21** (a recurrence means one regressed):
the vexa chart only renders a PDB when the component's `replicaCount > 1`, and
the CNPG singletons set `enablePDB: false`. `postgres16-primary`'s ALLOWED 0
PDB is **correct** — it forces the switchover flow above instead of a
primary eviction.

**Also expect to blink** when their node drains (no PDB, just sessions):
the rook-ceph toolbox-style pods, and the **agent pods (`dev-env`,
`dev-env-ops`)** — a drain of their node kills every live agent session.
Before starting a roll, note which node carries them
(`kubectl get pods -n dev -o wide`) and warn/pause agent work accordingly.
