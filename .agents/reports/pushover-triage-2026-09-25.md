...
| 09-26 04:31 | Airtime END OK — **66/66** idle(Gen1)→Gen4 bursts, first kernel ≤0.228 s, no CUDA errors; bursts 78 °C avg / 82 °C worst, sw_thermal 98 %, ~46→26 TFLOPS. Results on #3052 |
| 09-25 20:38 | (Tom's request) llama-server / Muse Glimmer restored on #1 (#3186); Rumpus Room Voice PE back on the Jarvis pipeline; 09-26: its wake word back to "Hey Jarvis" (the 09-23 change had also moved it to "Okay Nabu", so it answered the Movie Room) and the Jarvis agent given the Watch history (haynesnetwork) MCP API; `GpuMissing` runbook line fixed to target the node's pod (#3187) |
| 09-26 20:4x | 20 h quiet: 0 Pushover sends from Alertmanager, no criticals, #1 present. PR #3181 closed as superseded (auto-merge was still armed); B2 (triage guard wipe) and the 3rd sig re-key variant added to #3182 |
