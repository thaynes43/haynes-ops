...
| 5 | Git identity agents push as (personal key vs haynes-ops-bot vs new machine user) | **REVISED** 2026-07-13 (evening) | **Split identity.** haynes-ops-bot stays scoped to haynes-ops (ops/failure-reaction — the Shepherd's world). Dev-env agents use a NEW **haynes-dev-bot** GitHub App installed on All Repositories (contents+PRs+workflows write, checks read), same PEM→40min-token refresher pattern, 1P item `github-dev-bot` (same property names as `github-bot` so the mint script is unchanged). The dev pod carries ONLY the dev-bot credential. Provisioning is Tom's one-time ceremony; wiring PR held until the 1P item exists (an ES referencing a missing item wedges the ks NotReady). |
