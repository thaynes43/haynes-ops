...
1. **hass-sandbox** — branch, edit `appdaemon/`, bump `VERSION` (semver; **compare against `main` first** — `git show main:VERSION` — don't double-bump), run tests (`source .venv/bin/activate && cd appdaemon && python -m pytest tests/ -q`). Per its `AGENTS.md` (since hass-sandbox#129, 2026-09-04), PRs are opened **ready** — never `--draft`.
2. **Open + review** — `gh pr create` (ready; opening ready is what triggers the `Claude Code Review`, Agent Docs Audit and Docs Site Audit workflows — a draft skips them). Watch CI (`gh pr checks <N>`: required are `test`, `build-and-push`, `docs-build`), read the bot review + audits, and fix every finding or state on the PR why it does not apply. The audits and review can fail purely on the action's 25-turn budget ("reported a successful result after N turns"); that is not a finding and not a blocker, but it is not a reason to skip the findings they did post.
