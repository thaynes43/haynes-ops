...
| pve (Proxmox VE) | ✅ READ (exporter's PVEAuditor token) · ⚠️ OPERATOR **not wired yet** — needs all three steps in `externalsecret.yaml` (create the PVE token → add the 1Password `dev-env` fields → **append the commented-out `dev-env-proxmox-operator` ExternalSecret**, then bounce). Filling 1Password alone does nothing; the ExternalSecret does not exist yet | `pve ha` / `pve nodes` / `pve guests --onboot` / `pve vm <id> config\|onboot\|start\|stop\|reset --yes` / raw `pve get\|delete <path>`. Operator tier is **root-equivalent** on the 5-node PVE cluster (`Sys.Console`, Tom 2026-09-09) — by rule: no node reboots, no node shells, `vm` verbs refuse non-Talos guests without `--any`; `declare-activity` before anything disruptive. Runbook: `.agents/runbooks/proxmox-access.md` |
