...
# ── Codex: declarative config + the SAME rules and MCP servers as claude ────────
# config.toml = the GitOps file + `[mcp_servers.*]` rendered from claude's mcp.json
# by mcp-json-to-codex-toml.sh (ONE MCP list for both agents; by-name env
# references, so no secret is written to the file). AGENTS.md — codex's global
# instructions — = a codex preamble + the shared CLAUDE.md; per repo, config.toml's
# project_doc_fallback_filenames makes codex read a CLAUDE.md wherever no AGENTS.md
# exists. Both are regenerated every boot: edit the GitOps sources, never these.
if {
     cat "$CFG/codex/config.toml"
     printf '\n# ── MCP servers: GENERATED by dev-init from config/claude/mcp.json — edit mcp.json, not this ──\n\n'
     bash /opt/dev-env/scripts/mcp-json-to-codex-toml.sh "$CFG/claude/mcp.json"
   } > "$HOME/.codex/config.toml.tmp" && mv -f "$HOME/.codex/config.toml.tmp" "$HOME/.codex/config.toml"; then
  log "codex config.toml rendered ($(grep -c '^\[mcp_servers\.' "$HOME/.codex/config.toml") MCP servers from mcp.json)"
else
  rm -f "$HOME/.codex/config.toml.tmp"; log "WARN codex config.toml render failed — previous file left in place"
fi
if { cat "$CFG/codex/AGENTS.header.md"; printf '\n'; cat "$CFG/claude/CLAUDE.md"; } > "$HOME/.codex/AGENTS.md.tmp" \
   && mv -f "$HOME/.codex/AGENTS.md.tmp" "$HOME/.codex/AGENTS.md"; then
  log "codex AGENTS.md rendered (codex preamble + shared CLAUDE.md)"
else
  rm -f "$HOME/.codex/AGENTS.md.tmp"; log "WARN codex AGENTS.md render failed"
fi
