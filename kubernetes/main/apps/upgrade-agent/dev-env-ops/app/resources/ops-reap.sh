...
#   ops-reap.sh clean all       reap EVERY finished (done|failed) order now:
#                               kill its tmux window + remove ~/work/orders/<key>.json
#                               and the ~/work/<key> worktree/dir — no TTL wait
...
# failed >7d, bound-total >OPS_REAP_MAX_FINISHED); this exists so Tom or an
# agent can tidy on demand without waiting. Safe to run any time — it never
# touches pending/claimed orders or their windows.
set -uo pipefail
...
case "${1:-list}" in
  list)
    ...
    tmux list-windows -t ops -F '#W' 2>/dev/null | grep -E '^(wo|esc)-' | while read -r w; do
    ...
    if [ "$target" = "all" ]; then
      for key in $(printf '%s' "$data" | jq -r '
          to_entries[] | select((.value|fromjson? // {}) as $o
            | $o.status=="done" or $o.status=="failed") | .key' 2>/dev/null); do
        reap_key "$key"
      done
      ...
      st="$(status_of "$target")"
      case "$st" in
        done|failed) reap_key "$target"; echo "done." ;;
