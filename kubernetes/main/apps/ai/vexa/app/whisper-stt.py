...
def transcribe_file(
    ...
    if response_format is None:
        response_format = config.default_response_format
    # HAYNES-OPS OVERLAY (meeting-scribe): default a static name-bias lexicon
    # when the caller omits `hotwords`, so proper nouns (chapter name, brother
    # names) transcribe correctly. An explicit request-supplied `hotwords`
    # always wins — we only fill in the default when the field is absent/empty.
    # Source of the lexicon is env WHISPER_DEFAULT_HOTWORDS (set on the
    # Deployment); if unset, behaviour is identical to the stock image.
    if not hotwords:
        default_hotwords = os.environ.get("WHISPER_DEFAULT_HOTWORDS") or None
        if default_hotwords:
            hotwords = default_hotwords
            logger.info("Applying default hotwords bias from WHISPER_DEFAULT_HOTWORDS")
